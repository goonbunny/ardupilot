...
void AC_AttitudeControl::input_quaternion(Quaternion attitude_desired_quat, float smoothing_gain)
{
    ...
    attitude_controller_run_quat(0.0, 0.0, 0.0);
}
...
void AC_AttitudeControl::input_euler_angle_roll_pitch_yaw(float euler_roll_angle_cd, float euler_pitch_angle_cd, float euler_yaw_angle_cd, bool slew_yaw, float smoothing_gain)
{
    ...
    // Call quaternion attitude controller
    attitude_controller_run_quat(euler_roll_angle_cd, euler_pitch_angle_cd, euler_yaw_angle_cd);
}
...
void AC_AttitudeControl::attitude_controller_run_quat(float euler_roll_angle_cd, float euler_pitch_angle_cd, float euler_yaw_angle_cd)
{

    // Convert from centidegrees on public interface to radians
    float euler_roll_angle = radians(euler_roll_angle_cd*0.01f);
    float euler_pitch_angle = radians(euler_pitch_angle_cd*0.01f);
    float euler_yaw_angle = radians(wrap_180_cd(euler_yaw_angle_cd)*0.01f);


    float euler_roll_error = euler_roll_angle - _ahrs.roll;
    float euler_pitch_error = euler_pitch_angle - _ahrs.pitch;
    float euler_yaw_error = wrap_PI(euler_yaw_angle - _ahrs.yaw);
    ...
    attitude_error_vector.x = euler_roll_error;
    attitude_error_vector.y = euler_pitch_error;
    attitude_error_vector.z = euler_yaw_error;
   ...
   // _rate_target_ang_vel.y += -attitude_error_vector.x * _ahrs.get_gyro().z;
    ...
    if(AP_HAL::millis() - 50 > tnow) {
     //   ::printf("R: %.1f\t\tP: %.1f\t\tY: %.1f\nRe:%.1f\t\tPe:%.1f\t\tYe:%.1f\t", euler_roll_angle, euler_pitch_angle, euler_yaw_angle, euler_roll_error, euler_pitch_error, euler_yaw_error);
     //   ::printf("RTAVx:%.2f\t\tRTAVy:%.2f\t\tRTAVz:%.2f\t\t\n\n", _rate_target_ang_vel.x, _rate_target_ang_vel.y, _rate_target_ang_vel.z);
        tnow = AP_HAL::millis();
    }
...
/*
    ...
    */
/*
     float _roll_angle_rad = radians(wrap_180_cd(_ahrs.roll_sensor)*0.01f);
     float _temp_rate_target_ang_vel_y = _rate_target_ang_vel.y;

    _temp_rate_target_ang_vel_y = _rate_target_ang_vel.y * cosf(_roll_angle_rad) + _rate_target_ang_vel.z * sin(_roll_angle_rad);
    _rate_target_ang_vel.z = -_rate_target_ang_vel.y * sin(_roll_angle_rad) + _rate_target_ang_vel.z * cos(_roll_angle_rad);
    _rate_target_ang_vel.y = _temp_rate_target_ang_vel_y;
*/
    if (_rate_bf_ff_enabled & _use_ff_and_input_shaping) {
